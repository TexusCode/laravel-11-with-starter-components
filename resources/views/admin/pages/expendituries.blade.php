...
<div class="font-ALSHaussRegular">
    <x-primary-text text="Добавит новый расход" />
    <form action="{{ route('add-expenditure') }}" method="POST" enctype="multipart/form-data">
        @csrf
        <div class="flex items-center gap-4 mt-4">
            <div class="w-full grid gap-4">
                <x-default-input placeholder="Сумма" value="{{ old('price') }}" type="number" required="on" name="price"
                    class="col-span-full" />
                <x-default-input placeholder="Описание" value="{{ old('description') }}" type="text" required="on"
                    name="description" class="col-span-full" />
            </div>
    ...
    <div class="my-4 border-t-2">
        <x-primary-text text="Список расходов" class="my-4" />
        <div>
            <!-- Table -->
            <div
                            ...
                            <td class="h-px w-72 whitespace-nowrap">
                                <div class="px-6 py-3">
                                    <span class="block text-sm font-semibold text-gray-800">{{
                                        $expenditure->description }}</span>
                                </div>
                            </td>
                            <td class="size-px whitespace-nowrap">
                                <div class="px-6 py-1.5 flex gap-2 justify-end">
                                    <form action="{{ route('delete-expenditure', $expenditure->id) }}" method="POST">
                                        @csrf
