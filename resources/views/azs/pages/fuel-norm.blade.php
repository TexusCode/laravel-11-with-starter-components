...
<div class="flex flex-col items-center justify-center w-screen h-screen bg-black select-none">
    <div class="flex items-center gap-4 mb-5 text-4xl font-bold text-center text-white uppercase">
        <h1>дневная норма<br>цены на топливо</h1>
    </div>
    <div class="max-w-[500px] min-w-[500px] font-bold">
        <form id="myForm" action="{{ route('fuelnormpost') }}" method="POST"
            class="flex flex-col p-2 bg-yellow-500 rounded-xl">
            @csrf
            <label class="block w-full py-2 overflow-hidden rounded-md">
                <span class="text-xl font-medium text-black"> Бензин 92</span>
                <input type="text" name="benzin92" placeholder="" value="{{ $fuelnorm->benzin92 }}" required
                    class="w-full p-2 mt-3 text-xl border-none rounded-lg" />
            </label>
            <label class="block w-full py-2 overflow-hidden rounded-md">
                <span class="text-xl font-medium text-black"> Бензин 95</span>
                <input type="text" name="benzin95" placeholder="" value="{{ $fuelnorm->benzin95 }}" required
                    class="w-full p-2 mt-3 text-xl border-none rounded-lg" />
            </label>
            <label class="block w-full py-2 overflow-hidden rounded-md">
                <span class="text-xl font-medium text-black"> Газ </span>
                <input type="text" name="gas" value="{{ $fuelnorm->gas }}" placeholder="Абубакр" required
                    class="w-full p-2 mt-3 text-xl border-none rounded-lg" />
            </label>
            <label class="block w-full py-2 overflow-hidden rounded-md">
                <span class="text-xl font-medium text-black"> Дизель </span>
                <input type="text" name="diesel" value="{{ $fuelnorm->diesel }}" required
                    class="w-full p-2 mt-3 text-xl border-none rounded-lg" />
            </label>
            <button id="submitBtn" type="submit"
                class="py-2 mt-2 text-white duration-200 bg-black rounded-lg hover:bg-white hover:text-black">Обновить</button>
