...
<div class="flex flex-col items-center justify-center w-screen h-screen bg-black select-none">
    <div class="flex items-center gap-4 mb-5 text-4xl font-bold text-center text-white uppercase">
        <h1>Партнерская<br>программа</h1>
    </div>
    <div class="max-w-[500px] min-w-[500px] font-bold">
        <form id="myForm" action="{{ route('partnerpost') }}" method="POST"
            class="flex flex-col p-2 bg-yellow-500 rounded-xl">
            @csrf
            <label class="block w-full py-2 overflow-hidden rounded-md">
                <span class="text-xl font-medium text-black"> Qr- карта партнера</span>
                <input type="text" name="qr1" placeholder="Кр код человек который дадаите карта" value="" required
                    class="w-full p-2 mt-3 text-xl border-none rounded-lg" />
            </label>
            <label class="block w-full py-2 overflow-hidden rounded-md">
                <span class="text-xl font-medium text-black"> qr- карта второй</span>
                <input type="text" name="qr2" placeholder="Кр код карта который дадите" value="" required
                    class="w-full p-2 mt-3 text-xl border-none rounded-lg" />
            </label>

            <button id="submitBtn" type="submit"
                class="py-2 mt-2 text-white duration-200 bg-black rounded-lg hover:bg-white hover:text-black">Добавит</button>
